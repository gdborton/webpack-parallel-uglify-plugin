...
function createWorkers(count, maxListeners) {
  const workers = [];
...
function processAssets(compilation, options) {
  ...
  const maxListeners = Number.isInteger(options.maxListeners) ? options.maxListeners : 100;
  const workers = createWorkers(numWorkers, maxListeners);
