...
function minify(assetName, asset, worker, cacheDir, options) {
  const tmpFileName = tmpFile.create(JSON.stringify(asset.sourceAndMap()));
...
function processAssets(compilation, options) {
  ...
  for (let i = 0; i < assets.length; i++) {
    const name = assets[i];
    const cacheKey = cache.createCacheKey(assetHash[name].sourceAndMap(), optionsWithoutCacheDir);
    ...
    return minify(assetName, asset, worker, options.cacheDir, optionsWithoutCacheDir)
      .then(msgContent => {
        if (msgContent.map) {
          ...
          assetHash[assetName] = new RawSource(msgContent.code); // eslint-disable-line no-param-reassign, max-len
        }
      }).catch((e) => {
